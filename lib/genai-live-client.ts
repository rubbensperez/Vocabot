...
export class GenAILiveClient {
  ...
  private emitter: EventEmitter<LiveClientEventTypes>;
  ...
  constructor(apiKey: string, model?: string) {
    this.emitter = new EventEmitter<LiveClientEventTypes>();
  ...
  public addListener<T extends keyof LiveClientEventTypes>(
    event: T,
  ...
  public removeListener<T extends keyof LiveClientEventTypes>(
    event: T,
    listener: LiveClientEventTypes[T],
  ) {
  ...
  protected onMessage(message: LiveServerMessage) {
    ...
    if (message.serverContent) {
      ...
      if (serverContent.modelTurn) {
        ...
        base64s.forEach(b64 => {
          if (b64) {
            base64ToArrayBuffer(b64).then(data => {
